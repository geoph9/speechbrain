        ...
        if stage == sb.Stage.TRAIN:
            ...
            if hasattr(self.hparams, "augmentation"):
                try:
                    ...
                    print("Error in augmentation of id:", batch.id)
                    print("   with wav_lens:", wav_lens, "and wav shape:", wavs.shape)
                    print("   and text:", batch.wrd)
                    raise e
...
def dataio_prepare(hparams):
    ...
    # 2. Define audio pipeline.
    if hparams["mic_type"] == "Array1":
        # Multi-mic (Microphone Array)
        @sb.utils.data_pipeline.takes("wav", "start_time", "duration")
        @sb.utils.data_pipeline.provides("sig")
        def audio_pipeline(wav, start_time, duration):
            start_frame = int(start_time * hparams["sample_rate"])
            end_frame = start_frame + int(duration * hparams["sample_rate"])
            mics_signals = sb.dataio.dataio.read_audio_multichannel({
                "file": wav,
                "start": start_frame,
                "end": end_frame,
            }).unsqueeze(0)
            sig = hparams["multimic_beamformer"](mics_signals)
            sig = sig.squeeze()
            return sig
        ...
        @sb.utils.data_pipeline.provides("sig")
        def audio_pipeline(wav, start_time, duration):
            # Convert start_time from seconds to n_frames
            try:
                start_frame = int(float(start_time) * hparams["sample_rate"])
                n_frames = int(float(duration) * hparams["sample_rate"])
                # end_frame = start_frame + n_frames
            except Exception as e:
                print("Error in audio_pipeline")
                print("start_time: {}".format(start_time))
                print("duration: {}".format(duration))
                print("wav: {}".format(wav))
                raise e
            # Read the `wav` file and return the signal corresponding
            # to the specified start time and duration.
            # sig = sb.dataio.dataio.read_audio({
            #     "file": wav,
            #     "start": start_frame,
            #     "end": end_frame,
            # })
            sig, _ = torchaudio.load(wav, frame_offset=start_frame, num_frames=n_frames)
...
if __name__ == "__main__":
    ...
    from prepare_ami import prepare_ami  # noqa
    ...
    if not hparams["skip_prep"]:
        run_on_main(
            ...
            kwargs={
                ...
                "max_words_per_segment": hparams["max_words_per_segment"],
                "mic": hparams["mic_type"],
                "split_type": hparams["split_type"],
                "min_chars_per_segment": hparams["min_chars_per_segment"],
                "min_duration": hparams["min_duration"],
                "merge_consecutive": hparams["merge_consecutive"],
            },
        )
